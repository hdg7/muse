...
def evaluate_metric(options: Options) -> None:
    ...
    results = muse.run()
    # We assume outputs is a directory path and we will write the results to a file per summarizer system and evaluation method
    # We will also include the summaries generated by the summarizer system
    if options["output"]:
...
class Muse:
    ...
    def _evaluate_summarizer(self, summarizer: Summarizer) -> None:
        self.summary = summarizer.summarize(self.data)
        results = {}
