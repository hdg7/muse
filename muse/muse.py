...
def get_args():
    parser = argparse.ArgumentParser(description='MuSE is a evaluation system for summarization engines.')
    parser.add_argument('-s', '--system', help='The summarization system to evaluate', required=True)
    parser.add_argument('-t', '--type', help='The type of the summarization system (single-document, multi-document, conversation)', required=True)
    parser.add_argument('-d', '--data', help='The data to evaluate the summarization system on', required=True)
    parser.add_argument('-e', '--evType', help='The type of evaluation to perform (metrics, reference, llms)', required=True)
    parser.add_argument('-m', '--metrics', help='The metrics to use for evaluation')
    parser.add_argument('-c', '--config', help='The configuration file to use for evaluation')
